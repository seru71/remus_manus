---
title: "Overcview of Remus regulatory datasets"
output:
...
scr_cl_median = median(screen_chr38$size[screen_chr38$tissue_type=='primary cell'])
scr_ub_median = median(screen_chr38$size[screen_chr38$tissue_type=='tissue'])

```

Open chromatin regions for a primary cell in ENCODE and SCREEN datasets have on average `r mean(enc_chr38[enc_chr38$tissue_type=='primary cell',]$size) / 1000000` Mb and `r mean(screen_chr38[screen_chr38$tissue_type=='primary cell',]$size) / 1000000` Mb, respectively. Median size of a primary cell track constitutes `r 100*enc_cl_median/all_enc_chr` %  and `r 100*scr_cl_median/all_scr_chr` % of the tissue-unselected open chromatin in ENCODE and SCREEN respectively.

For tissues, open chromatin regions consitute on average `r mean(enc_chr38[enc_chr38$tissue_type=='tissue',]$size) / 1000000` and `r mean(screen_chr38[screen_chr38$tissue_type=='tissue',]$size) / 1000000` Mb respectively. Median size of a tissue track constitutes `r 100*enc_ub_median/all_enc_chr` %  and `r 100*scr_ub_median/all_scr_chr` % of the tissue-unselected open chromatin in ENCODE and SCREEN respectively.
...
plot(s1_shared$size/1000000, s2_shared$size/1000000, xlab="SCREEN (Mb)", ylab="FANTOM5 (Mb)", pch=4, asp=1, las=2)
abline(a=0,b=1)
...
all_f5_tss = at$size[at$type=='tss' & at$src=='fantom5' & at$genome=='GRCh38']
...
all_scr_tss = at$size[at$type=='tss' & at$src=='screen' & at$genome=='GRCh38']
scr_cl_median = median(screen_tss38$size[screen_tss38$tissue_type=='primary cell'])
scr_ub_median = median(screen_tss38$size[screen_tss38$tissue_type=='tissue'])

```


Promoter regions for a primary cell in SCREEN and FANTOM5 datasets have on average `r mean(screen_tss38[screen_tss38$tissue_type=='primary cell',]$size) / 1000000` Mb and `r mean(f5_tss38[f5_tss38$tissue_type=='primary cell',]$size) / 1000000` Mb, respectively. Median size of a primary cell track constitutes `r 100*scr_cl_median/all_scr_tss` %  and `r 100*f5_cl_median/all_f5_tss` % of the tissue-unselected promoters in SCREEN and FANTOM5 respectively.

For tissues, promoter regions consitute on average `r mean(screen_tss38[screen_tss38$tissue_type=='tissue',]$size) / 1000000` and `r mean(f5_tss38[f5_tss38$tissue_type=='tissue',]$size) / 1000000` Mb for SCREEN and FANTOM5 respectively. Median size of a tissue track constitutes `r 100*scr_ub_median/all_scr_tss` %  and `r 100*f5_ub_median/all_f5_tss` % of the tissue-unselected promoters in SCREEN and FANTOM5 respectively.
...
tissues <- list(chromatin = unique(chr38$tissue_ID),
                enhancers = unique(enh38$tissue_ID),
                promoters = unique(tss38$tissue_ID))
fig2a = plot(venn(tissues), quantities=list(cex = .8), legend=list(cex=0.8), main = "(a) All regulators")

tissue_ids <- list(ENCODE = unique(enc_enh38$tissue_ID),
                   FANTOM5= f5_enh38$tissue_ID,                                                   
                   SCREEN = unique(screen_enh38$tissue_ID))
fig2b <- plot(euler(tissue_ids), quantities=list(cex = .8), legend=list(cex=0.8), main = "(b) Enhancer tracks")
...
fig3b <- ggplot(enh38[!enh38$embryonic,], aes(x=src, y=pct.of.all, fill=tissue_type)) +
  geom_boxplot() + theme(text=element_text(size=8), legend.position = "none") + ylim(0,0.4) +
...
fig3a <- ggplot(tss38[!tss38$embryonic,], aes(x=src, y=pct.of.all, fill=tissue_type)) +
  geom_boxplot() + theme(text=element_text(size=8), legend.position = "none") +
...
fig3c <- ggplot(chr38[!chr38$embryonic,], aes(x=src, y=pct.of.all, fill=tissue_type)) +
  geom_boxplot() + theme(text=element_text(size=8)) + ylim(0,0.4) +
  ggtitle("(c) Open chromatin") + xlab("") + ylab("")
...
a <- ggplot(enh38, aes(x=src, y=log10(cnt), fill=tissue_type)) + 
  geom_boxplot() + theme(text=element_text(size=8), legend.position = "none") +
  ggtitle("(a) TFBS") + xlab("") + ylab("log10(count)")

b <- ggplot(enh38, aes(x=src, y=size/cnt, fill=tissue_type)) + 
  geom_boxplot() + theme(text=element_text(size=8), legend.position = "none") +
  ggtitle("(b) TFBS") + xlab("") + ylab("Average region size (bp)")

c <- ggplot(tss38[!tss38$embryonic,], aes(x=src, y=log10(cnt), fill=tissue_type)) + 
  geom_boxplot() + theme(text=element_text(size=8), legend.position = "none") +
...
e <- ggplot(chr38[!chr38$embryonic,], aes(x=src, y=log10(cnt), fill=tissue_type)) +
  geom_boxplot() + theme(text=element_text(size=8)) +
  ggtitle("(e) Open chromatin") + xlab("") + ylab("")

f <- ggplot(chr38[!chr38$embryonic,], aes(x=src, y=size/cnt, fill=tissue_type)) +
  geom_boxplot() + theme(text=element_text(size=8)) +
  ggtitle("(f) Open chromatin") + xlab("") + ylab("")
...
dfhg38 <- enc_enh[enc_enh$genome == "GRCh38",]
dfhg38wlo <- enc_enh[enc_enh$genome == "GRCh38_with_liftover",]
barplot((dfhg38wlo$size-dfhg38$size)*100/dfhg38$size, 
        las=2, names.arg = dfhg38$tissue_ID, cex.names = 0.6, 
        main='ENCODE TF-binding [%]', ylab="Gain in % bp")

lo.diff38 = merge(enc_chr[enc_chr$genome == "GRCh38" & !enc_chr$embryonic,], 
                enc_chr[enc_chr$genome == "GRCh38_with_liftover"  & !enc_chr$embryonic,],
                by="tissue_ID",all=T)
barplot((lo.diff38$size.y-lo.diff38$size.x)*100/lo.diff38$size.x,
        names.arg = lo.diff38$tissue_ID, las=2, cex.names = 0.5, 
        main = "ENCODE accessible chromatin", ylab="Gain in % bp")

summary(lo.diff38$size.y-lo.diff38$size.x)
summary((lo.diff38$size.y-lo.diff38$size.x)*100/lo.diff38$size.x)
```