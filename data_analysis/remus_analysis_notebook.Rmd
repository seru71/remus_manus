...
fig3b <- ggplot(enh38[!enh38$embryonic,], aes(x=src, y=pct.of.all, fill=tissue_type)) +
  ...
  scale_x_discrete(labels=c('ENCODE', 'FANTOM', 'SCREEN'))
...
fig3a <- ggplot(tss38[!tss38$embryonic,], aes(x=src, y=pct.of.all, fill=tissue_type)) +
  ...
  scale_x_discrete(labels=c('FANTOM', 'SCREEN'))
...
fig3c <- ggplot(chr38[!chr38$embryonic,], aes(x=src, y=pct.of.all, fill=tissue_type)) +
  ...
  scale_x_discrete(labels=c('ENCODE', 'SCREEN'))
...
chr.name = paste0('ENCODE accessible chromatin\n(N=', sum(lo.diff.chr38$size.y-lo.diff.chr38$size.x>0),")")
tfbs.name = paste0('ENCODE TFBS\n(N=', sum(lo.diff.enh38$size.y-lo.diff.enh38$size.x>0),")")
df = rbind(data.frame(inc = (lo.diff.enh38$size.y-lo.diff.enh38$size.x)*100/lo.diff.enh38$size.x, type=tfbs.name),
           data.frame(inc = (lo.diff.chr38$size.y-lo.diff.chr38$size.x)*100/lo.diff.chr38$size.x, type=chr.name))

df$inc[df$inc==0] <- NA
#df$inc <- sapply(df$inc, function(x) max(x, 0.01))
ggplot(df, aes(x=type, y=inc)) + 
  geom_boxplot() + ylab("Gain in % bp (log scale)") + xlab("") + scale_y_log10(breaks=c(0.3,1,2,5,10,20,50,100,150,200,300)) +
...
tss.df = rbind(data.frame(tissue_type=df$tissue_type, bp_overlap=df$set1_overlap_frac2, src="SCREEN"), 
               data.frame(tissue_type=df$tissue_type, bp_overlap=df$set2_overlap_frac2, src="FANTOM5"))

color = as.integer(as.factor(df$tissue_type))
plot(x=df$set1_overlap_frac2*100, y=df$set2_overlap_frac2*100, col=color, pch=4,
     xlab="Basepair overlap as fraction of SCREEN promoters", 
     ylab="Basepair overlap as fraction of FANTOM5 promoters")
text(x=df$set1_overlap_frac2*100, y=1+df$set2_overlap_frac2*100, labels=df$tissue, 
     cex=0.6, col=color)
legend("bottom", legend=levels(factor(df$tissue_type)), pch=4, col=c(1,2))

a <- ggplot(tss.df, aes(x=src, y=bp_overlap, fill=tissue_type)) + geom_boxplot() +
     theme(legend.position = "none") + ylim(0,1) +
     ggtitle("(a) Promoters") + xlab("") + ylab("Fraction of basepair overlap") 
...
set1 = "screen"
set2 = "encode"
shared_tissues = which(apply(table(chr38[, c("src","tissue_ID")])[c(set1,set2),],2,prod)>0)
overlap_subset = chr38[!chr38$embryonic & chr38$tissue_ID %in% names(shared_tissues),c("src","tissue_ID","tissue", "tissue_type")]
os1 = overlap_subset[overlap_subset$src==set1,]
os2 = overlap_subset[overlap_subset$src==set2,]
fnames1 = paste0("chromatin/screen/GRCh38_with_liftover/", os1$tissue_ID, "_", os1$tissue, ".bed.gz")
fnames2 = paste0("chromatin/encode/GRCh38_with_liftover/", os2$tissue_ID, "_", os2$tissue, ".bed.gz")


df <- data.frame(tissue=os1$tissue, tissue_type=os2$tissue_type, 
                 screen=fnames1, encode=fnames2, 
                 set1_overlap_cnt=0, set2_overlap_cnt=0,
                 set1_overlap_frac1=0, set1_overlap_frac2=0,
                 set2_overlap_frac1=0, set2_overlap_frac2=0, 
                 stringsAsFactors = F)
...
color = as.integer(as.factor(df$tissue_type))
plot(x=df$set1_overlap_frac2*100, y=df$set2_overlap_frac2*100, col=color, pch=4,
     xlab="Basepair overlap as fraction of SCREEN accessible chromatin", 
     ylab="Basepair overlap as fraction of ENCODE accessible chromatin")
text(x=df$set1_overlap_frac2*100, y=1+df$set2_overlap_frac2*100, labels=df$tissue, 
     cex=0.6, col=color)
legend("topright", legend=levels(factor(df$tissue_type)), pch=4, col=c(1,2))


chr.df = rbind(data.frame(tissue_type=df$tissue_type, bp_overlap=df$set1_overlap_frac2, src="SCREEN"), 
            data.frame(tissue_type=df$tissue_type, bp_overlap=df$set2_overlap_frac2, src="ENCODE"))


b <- ggplot(chr.df, aes(x=src, y=bp_overlap, fill=tissue_type)) + geom_boxplot() + ylim(0,1) +
     ggtitle("(b) Accessible chromatin") + xlab("") + ylab("") + labs(fill="Track type")


grid.arrange(a,b,ncol=2, widths=c(2,3))
